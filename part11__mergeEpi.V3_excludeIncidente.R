...
options(future.globals.maxSize=150*1024^3)# memory 100GB
# working.directory: 
setwd("/ ... ")

# saveRDS(epi.tumor, "epi.tumor.V3.rds")
# epi.tumor=readRDS("epi.tumor.V3.rds")
...
rownames2=rownames(prime@assays$RNA@data)
rownames3=rownames(crpce@assays$RNA@data)
# rownames4=rownames(incie@assays$RNA@data)
str(rownames1)
# chr [1:27984] "MIR1302-2HG" "FAM138A" "OR4F5" "AL627309.1" "AL627309.3" ...
str(rownames3)
# chr [1:27984] "MIR1302-2HG" "FAM138A" "OR4F5" "AL627309.1" "AL627309.3" ...
epi=merge(x = normale, y = list(prime=prime, crpce=crpce)) # incidente=incie
# str(epi)
...
# crC6 crCy3  crL0  crL1  crL2 crNE5   nB2   nB5   nC3   nL0   nL1   nN8  tB21
#   70   795  1972  1755  1695   132  2925   280  1299  3705  3561    19   652
# tC20  tC23  tC24   tL0   tL1  tL22   tL3   tL4   tL5
#  894   171    31  4069  3853   577  1659  1446  1289
table(is.na(epi@meta.data$epitype))
# FALSE
# 32849
table(epi@meta.data$celltype)
# Basal CellCycle     Clike   Luminal        NE     TypeC
#  3857       795       241     25581       151      2224
...
table(epi@meta.data$pheno)
  # ADT  CRPC mHSPC  NEPC  Norm  Prim
  # 327  6250  6316   169 11789  7998
mHSPC=c( "DHB-T", "HYQ", "LPX", "QLX-M", "RSC1023" )
...
epi@meta.data$temp=factor(epi@meta.data$temp, levels=c("Normal prostate", "Primary PCa", "mHSPC", "post-ADT", "CRPC", "NEPC"))
epi@meta.data$fig.pheno=epi@meta.data$temp
table(epi@meta.data$fig.pheno)
# Normal prostate     Primary PCa           mHSPC        post-ADT            CRPC
#           11789            6970            7344             327            6250
#            NEPC
#             169
table(is.na(epi@meta.data$fig.pheno))
# FALSE
# 32849

### inferCNV by "part11__inferCNV.onlyEpithelial.V5.excludeIncidente.R"
{
  ### read inferCNV result (based on $SCT@counts), and classify cancer and normal cells

  ### downsampled epithelial cells's result with "nL1" "nB2" cells as reference is under "../inferCNV/allcelltypes.excludeIncidente.sct.counts.out/", this is the best downsampled result

  # epi=readRDS("epi.fullsamples.V3.rds")
  cnv=list()
  cnv[[1]]=read.table("../inferCNV/epitype.V5.sct.counts.out/infercnv.observations.txt")
  cnv[[2]]=read.table("../inferCNV/epitype.V5.sct.counts.out/infercnv.references.txt")
  for(i in 1:2)
  {
  ...
  str(cnv)
  cnv=c(cnv[[1]], cnv[[2]])
  # saveRDS(cnv, "cnv.intensity.epitype.V5.sct.counts.rds")
  # cnv=readRDS("cnv.intensity.epitype.V5.sct.counts.rds")
  ...
  table(is.na(epi@meta.data$cnv.counts))
  ...
  # "nL1" "nB2" cells as reference for inferCNV
  norm.median=median(epi@meta.data$cnv.counts[(epi@meta.data$fig.pheno%in%c("Normal prostate")) & (!epi@meta.data$epitype%in%c("nL1", "nB2"))])
  # norm.median=median(epi@meta.data$cnv.counts[ epi@meta.data$fig.pheno%in%c("Normal prostate") ])
  temp.median
  # 0.002094243
  norm.median
  # [1] 0.001027353
  ...
  # tC20  tC23  tC24   tL0   tL1  tL22   tL3   tL4   tL5
  #  894   171    31  4069  3853   577  1659  1446  1289
  table((epi@meta.data$fig.pheno%in%c("Normal prostate")), (!epi@meta.data$epitype%in%c("nL1", "nB2")))
  #         FALSE  TRUE
  # FALSE     0 21060
  # TRUE   6486  5303

  print(median(epi@meta.data$cnv.counts[epi@meta.data$epitype%in%c("nB2")]))
  print(median(epi@meta.data$cnv.counts[epi@meta.data$epitype%in%c("nB5")]))
  print(median(epi@meta.data$cnv.counts[epi@meta.data$epitype%in%c("nC3")]))
  print(median(epi@meta.data$cnv.counts[epi@meta.data$epitype%in%c("nL0")]))
  print(median(epi@meta.data$cnv.counts[epi@meta.data$epitype%in%c("nL1")]))
  print(median(epi@meta.data$cnv.counts[epi@meta.data$epitype%in%c("nN8")]))
  # 0.001251672 ----highest in normal epithelial
  print(median(epi@meta.data$cnv.counts[epi@meta.data$epitype%in%c("tC24")]))
  print(median(epi@meta.data$cnv.counts[epi@meta.data$epitype%in%c("tC23")]))
  print(median(epi@meta.data$cnv.counts[epi@meta.data$epitype%in%c("tB21")]))
  # 0.001286192 lowest in primary/mHSPC/post-ADT
  wilcox.test(epi@meta.data$cnv.counts[epi@meta.data$epitype%in%c("tB21")], norm.median, alternative="greater")
  # c("two.sided", "less", "greater")
  # p-value = 0.1829
  wilcox.test(epi@meta.data$cnv.counts[epi@meta.data$epitype%in%c("tB21")], epi@meta.data$cnv.counts[epi@meta.data$epitype%in%c("nB5", "nC3", "nL0", "nN8")], alternative="two.sided")
  # p-value < 2.2e-16
  wilcox.test(epi@meta.data$cnv.counts[epi@meta.data$epitype%in%c("nN8")], epi@meta.data$cnv.counts[epi@meta.data$epitype%in%c("nB5", "nC3", "nL0", "nN8")], alternative="two.sided")
  # p-value = 0.003787
  wilcox.test(epi@meta.data$cnv.counts[epi@meta.data$epitype%in%c("tB21")], epi@meta.data$cnv.counts[epi@meta.data$epitype%in%c("nN8")], alternative="two.sided")
  # p-value = 0.8779
  wilcox.test(epi@meta.data$cnv.counts[epi@meta.data$epitype%in%c("nC3")], epi@meta.data$cnv.counts[epi@meta.data$epitype%in%c("nB5", "nC3", "nL0", "nN8")], alternative="two.sided")
  # p-value < 2.2e-16
  ...
  temp=temp+geom_hline(yintercept = norm.median, col = 'red', size = 0.5)# dashed line: , linetype = 'dotted'
  print(temp)
  dev.off()
...
# saveRDS(epi, "epi.fullsamples.V3.rds")
# epi=readRDS("epi.fullsamples.V3.rds")
...
{ 
  ...
  table(epi@meta.data$fig.pheno)
  # Normal prostate     Primary PCa           mHSPC        post-ADT            CRPC
  #         11945            6970            7344             327            6250
  #          NEPC
  #           169
  epi.tumor=subset(epi.tumor, subset=fig.pheno%in%c("Primary PCa", "mHSPC", "post-ADT", "CRPC", "NEPC"))
  print(dim(epi.tumor))
  # [1] 21249 21060 
...
# ADT  CRPC mHSPC  NEPC  Prim
# 327  6250  6316   169  7998
mHSPC=c( "DHB-T", "HYQ", "LPX", "QLX-M", "RSC1023" )
epi.tumor@meta.data$temp <- epi.tumor@meta.data$pheno
epi.tumor@meta.data$temp[epi.tumor@meta.data$temp%in%c("Prim", "mHSPC")]="Primary PCa"
epi.tumor@meta.data$temp[epi.tumor@meta.data$fig.patient%in%mHSPC]="mHSPC"
epi.tumor@meta.data$temp[epi.tumor@meta.data$temp=="ADT"]="post-ADT"
epi.tumor@meta.data$temp=factor(epi.tumor@meta.data$temp, levels=c("Primary PCa", "mHSPC", "post-ADT", "CRPC", "NEPC"))
table(is.na(epi.tumor@meta.data$temp))
# FALSE
# 21060
epi.tumor@meta.data$fig.pheno=epi.tumor@meta.data$temp
table(epi.tumor@meta.data$fig.pheno)
# Primary PCa       mHSPC    post-ADT        CRPC        NEPC
#       10352        3962         327        6250         169
table(is.na(epi.tumor@meta.data$fig.pheno))
# FALSE
# 32849
table(epi.tumor@meta.data$fig.pheno, epi.tumor@meta.data$fig.patient)


epi.tumor=SCTransform(epi.tumor, vars.to.regress = c("percent.mt"), verbose = FALSE, return.only.var.genes = FALSE, do.scale=TRUE, method="glmGamPoi")# glmGamPoi is used 
DefaultAssay(epi)="SCT"
markerlist.epi.tumor=list()
celltype=sort(unique(epi.tumor@meta.data$epitype))
...
for(i in 1:(length.celltype-1))
{
  for(j in ((i+1):length.celltype))
  {
    ...
    markerlist.epi.tumor[[paste(ci, cj, sep="__")]]=FindMarkers(epi.tumor, ident.1 = ci, ident.2 = cj, assay="SCT")
  }
}
# saveRDS(markerlist.epi.tumor, "epi.tumor.markerlist.pairwise.V3.rds")
# markerlist.epi.tumor=readRDS("epi.tumor.markerlist.pairwise.V3.rds")
...
### Harmony-based clustering (parameters adjusted), pair-wise DEGs as var genes 
{
  ...
  print(unicluster)
  for(i in names(markerlist.epi.tumor))
  {
    j=markerlist.epi.tumor[[i]]
    mn=strsplit(i, split="__")[[1]]
    m=mn[1]
    n=mn[2]
    if( (m%in%unicluster)&(n%in%unicluster) )
    {
      # geneset=union( geneset, rownames(j)[ (j$p_val_adj<0.05) & ((j$avg_log2FC)>log2(2)) ] )
      # geneset=union( geneset, rownames(j)[ (j$p_val_adj<0.05) & (abs(j$avg_log2FC)>log2(1.5)) ] )
      # geneset=union( geneset, rownames(j)[ (j$p_val_adj<0.05) & ((j$avg_log2FC)>log2(1.5)) ] )
      geneset=union( geneset, rownames(j)[ (j$p_val_adj<0.05) & (abs(j$avg_log2FC)>log2(2)) ] )
    }
  }
  print("str(geneset):")
  str(geneset)
  # log2FC>2 chr [1:1999] "GATA3" "PIGR" "KRT7" "VGLL1" "LCN2" "CX3CL1" "CLIC6" "UPK1A" ...
  # absLog2FC>1.5 chr [1:4971] "GATA3" "PIGR" "KRT7" "VGLL1" "LCN2" "CX3CL1" "CLIC6" "UPK1A" ... 
  # Log2FC>1.5 chr [1:4492] "GATA3" "PIGR" "KRT7" "VGLL1" "LCN2" "CX3CL1" "CLIC6" "UPK1A" ...
  # absLog2FC>2  chr [1:2278] "GATA3" "PIGR" "KRT7" "VGLL1" "LCN2" "CX3CL1" "CLIC6" "UPK1A" ...
  ...
  epi.tumor = SetIdent(epi.tumor, value = "epitype")
  table(epi.tumor@active.ident)
  # tC20   tL3   tL0  tB21   tL1  tL22   tL4   tL5  tC23  tC24 crCy3  crL1  crL2
  #  894  1659  4069   652  3853   577  1446  1289   171    31   795  1755  1695
  # crL0 crNE5  crC6
  # 1972   132    70
...
check.genes=c( "nFeature_RNA", "nFeature_SCT", "PSCA")


DefaultAssay(epi.tumor)="SCT"
png("temp.featureplot.png", height=12*100, width=14*100)
FeaturePlot(epi.tumor, features = check.genes)
dev.off()
DefaultAssay(epi.tumor)="SCT"
png("temp.VlnPlot.png.png", width=10*100, height=8*100)
epi=SetIdent(epi.tumor, value="monocle.state")
temp=VlnPlot(epi.tumor, features = check.genes, pt.size = 0, group.by="monocle.state", ncol = 2)
for(i in 1:length(check.genes)) 
{
...
pdf("epi.tumor.celltype.dimplot.pdf", width=5, height=4.5)
DimPlot(epi.tumor, label = FALSE, group.by="celltype", raster = TRUE, order=oorder, cols=color)
# + NoLegend()  
dev.off()
pdf("epi.tumor.epitype.dimplot.pdf", width=5, height=4.5)
DimPlot(epi.tumor, label = FALSE, group.by="epitype", raster = TRUE, )# + NoLegend()
dev.off()
...
DefaultAssay(epi.tumor)="SCT"
tocheck=c( "IL1RN", "IL1A",  "PSCA", "KRT4" ,   "KRT5",   "KLK3", "AR",  "TOP2A", "STMN1",    "ENO2" )
tocheck=c( "PSCA", "FGFR3", "EPCAM", "KRT4", "KRT5", "KLK3", "AR",  "ENO2", "NCAM1", "CEACAM1" )
 # "NCAM1", "EPCAM", "CEACAM1"
...
pdf("epi.tumor_VlnPlot_celltype.pdf", width = 5, height =24)
...
### f2f
pdf("epi.tumor.dimplot.phenotype.pdf", height=5, width=6)
...
### qusage to find activated pathways
{
  ...
  epi.tumor@meta.data$for_temp_color=as.numeric(epi.tumor@meta.data$factor.fig.celltype)
  levels(epi.tumor@meta.data$factor.fig.celltype)
  run_qusage_heatmap.seurat3(epi.tumor, nm = 'kegg.celltype', kegg.list, my.seed=100)
  run_qusage_heatmap.seurat3(epi.tumor, nm = 'hall.celltype', hall.list, my.seed=100)
}


### enrichment with susu.enrich.plot
{
  ### celltype-specific DEG identification
  {
    ...
    markerlist=list()
    celltype=unique(epi.tumor@meta.data$celltype)
    celltype=sort(celltype)
    epi.tumor = SetIdent(epi.tumor, value = "celltype")
    for(i in celltype)
    {
      for(j in celltype)
      {
          if(i!=j)
          {
            ...
            markerlist[[temp.name]]=FindMarkers(epi.tumor, ident.1 = i, ident.2 = j, slot="data", assay="SCT")
          }
      }
    }
    # saveRDS(markerlist, "epi.tumor.celltype.pairwise.V4.rds")
    # markerlist=readRDS("epi.tumor.celltype.pairwise.V4.rds")
  }  
  ...
  ### cannonical process to find specific markers of Clike cells
  {
    ...
    for(i in celltype)
    {
      temp.genes=rownames(epi.tumor@assays$SCT@data)
      for(j in celltype)
      {
          if(i!=j)
          {
            ...
            cur.genes=rownames(cur.list[cur.list$avg_log2FC>log2(2.5) & cur.list$pct.1>0.4, ] ) 
            # cur.genes=rownames(cur.list[cur.list$avg_log2FC>log2(2) & cur.list$pct.1>0.3 & cur.list$pct.2<0.1,])
            # cur.genes=rownames(cur.list[cur.list$avg_log2FC>log2(1.5),]) # used to get intersection of prime and crpce's Clike specific markers
    ...
    sp.markerlist=sp.markerlist[c("Clike")]
    tocheck=Reduce(union, sp.markerlist)
    # tocheck=c( tocheck, c("CD55", "FGFR3", "TACSTD2") ) # ITGA2:CD49B ITGA1:CD49A CD24
    temp <- SetIdent(temp, value = "celltype")
    levels(temp)=c("Clike", "TypeC", "Basal", "Luminal", "CellCycle", "NE")
    pdf(paste("heatmap.epi.tumor.typeC.pdf", sep=""),  width=10, height=length(tocheck)/3)
    ...
    sp.markerlist
    table(c("CD55", "IL1RN", "IL1A", "UPK1A", "UPK1B", "PIM1", "PMAIP1")%in%sp.markerlist[["Clike"]])

    epi.tumor.clike.markers=sp.markerlist$Clike
    str(epi.tumor.clike.markers)
    # chr [1:24] "S100A4" "IL1A" "IL1RN" "UPK1B" "S100P" "AREG" "HPGD" "SPINK1" ...

    epi.tumor.clike.markers
    #  [1] "S100A4"!!! "IL1A"   "IL1RN"  "UPK1B"  "S100P"  "AREG"   "HPGD"   "SPINK1"
    #  [9] "FOXQ1"  "TRIM31" "PIM1"   "FABP4"  "PSCA"   "UPK2"   "AKR1C2" "SNCG"
    # [17] "ADIRF"  "DHRS2"  "GPX2"   "BMP2"   "UPK1A"  "UPK3A"!!!  "FBLN1"  "CSTB"
    enrichment.gene.set.plot(epi.tumor.clike.markers, topNterm=20)

  }
}


### enrichment: good enough, not need to use comparison in primary PCa's cell types
{
  markerlist=readRDS("epi.tumor.celltype.pairwise.V4.rds")
  ...
  str(temp)
  enrichment.gene.set.plot(rownames(temp), topNterm=20, prefix="Clike_typeC.temp.enrich")

  temp=markerlist[["Clike__Luminal"]]
  temp=temp[temp$avg_log2FC>log2(1.5)  & temp$pct.1>0.1 & temp$p_val_adj<0.05, ] #  
  str(temp)
  enrichment.gene.set.plot(rownames(temp), topNterm=15, prefix="Clike_luminal.temp.enrich")
}
...
### monocle 2
{
  ...
  # epi.tumor=readRDS("epi.tumdor.V3.rds")
  temp=epi.tumor
  temp.levels=levels(temp@meta.data$fig.pheno)
  temp.levels=temp.levels[2:length(temp.levels)]
  temp@meta.data$fig.pheno=as.character(temp@meta.data$fig.pheno)
  temp@meta.data$fig.pheno=factor(temp@meta.data$fig.pheno, levels=temp.levels)
  table(temp@meta.data$fig.pheno)
  table(is.na(temp@meta.data$fig.pheno))
  # FALSE
  # 21060
  # selected=colnames(temp)[temp@meta.data$fig.pheno%in%]
  # temp=subset(temp, cells=)
  ...
  ### use all significant pair-wise epitype's DEGs as ordering genes
  {
    ...
    # chr [1:1999] "LCN2" "UPK1A" "PSCA" "CD74" "SPINK1" "S100A9" "DHRS2" ...
    # (j$p_val_adj<0.05) & ( abs(j$avg_log2FC)>log2(2) ) :
    # chr [1:2278] "LCN2" "UPK1A" "PSCA" "CD74" "SPINK1" "S100A9" "DHRS2" ...
    # (j$p_val_adj<0.05) & ( (j$avg_log2FC)>log2(1.5) ) :
    # chr [1:4492] "LCN2" "UPK1A" "PSCA" "CD74" "SPINK1" "S100A9" "DHRS2" ...
  ...
  pdf("temp_trajectory_state.pdf")
  temp=plot_cell_trajectory(temp.m, color_by = "State") #+facet_wrap(~State, nrow = 1)
  print(temp)
  dev.off()
  ...
  pdf("temp_trajectory_pheno.pdf")
  temp=plot_cell_trajectory(temp.m, color_by = "fig.pheno")
  print(temp)
  dev.off()

  pdf("temp_trajectory_pheno.pdf")
  temp=plot_cell_trajectory(temp.m, color_by = "fig.pheno") +facet_wrap(~fig.pheno, nrow = 1)
  print(temp)
  dev.off()

  table(pData(temp.m)$fig.pheno)

  GM_state <- function(temp.m){
    if (length(unique(pData(temp.m)$State)) > 1)
    {
      T0_counts <- table(pData(temp.m)$State, pData(temp.m)$fig.pheno)[,"post-ADT"]
  ...
  table(rownames(pData(temp.m))==rownames(epi.tumor@meta.data))
  #  TRUE
  # 21060
  epi.tumor@meta.data$monocle.state=pData(temp.m)$State
  # pData(temp.m)$State=pData(temp.m)$fig.celltype
  temp.m <- orderCells(temp.m, root_state = GM_state(temp.m))
  # saveRDS(temp.m, "epi.tumor.monocle.V3.allpheno.rds")
  # temp.m=readRDS("epi.tumor.monocle.V3.allpheno.rds")
  ...
  # png("plot_cell_trajectory_byPseudotime_temp.m.png")
  temp=plot_cell_trajectory(temp.m, color_by = "factor.fig.celltype", cell_size=0.1)
  print(temp)
  dev.off()

  pdf("plot_cell_trajectory_fig.pheno_temp.m.pdf") # , width=5, height=5
  # png("plot_cell_trajectory_byPseudotime_temp.m.png")
  temp=plot_cell_trajectory(temp.m, color_by = "fig.pheno", cell_size=0.1)# cell_name_size = 8
  print(temp)
  dev.off()
  ...
  pdf("plot_cell_trajectory_details_fig.celltype_temp.m.pdf", width=29, height=6)
  temp=plot_cell_trajectory(temp.m, color_by = "factor.fig.celltype", cell_size=0.2) +
      facet_wrap(~fig.celltype, nrow = 1)
  print(temp)
  dev.off()

  pdf("plot_cell_trajectory_details_fig.celltype.pheno_temp.m.pdf", width=25, height=6)
  temp=plot_cell_trajectory(temp.m, color_by = "factor.fig.celltype", cell_size=1) +
  ...
  ### barplot the percentage of each phenotype in states
  {
    ...
    table(names(temp.m.state)==rownames(epi.tumor@meta.data))
    epi.tumor@meta.data[names(temp.m.state), "State"]=temp.m.state
    table(is.na(epi.tumor@meta.data$State))
    ...
    # [1] "1" "2" "3" "4" "5" "6" "7"
    new.cluster.ids <- c("State associated with reccurence", "State associated with reccurence", "State associated with reccurence", "State associated with reccurence", "State associated with reccurence", "State responsive to ADT", "Starting state")
    ...
    table(is.na(epi.tumor@meta.data$State3))
    ...
    require("scales")
    color=hue_pal()(n_top+1)[2:(n_top+1)]
  ...
  ### boxplot Clike percentage for CRPC+NEPC vs mHSPC vs Primary and incidental aggressive vs indolent
  {
    # give sample ID as phenotype + number
    {
      ...
      pdf("epi.tumor.pheno.sample.dimplot.pdf", width=5, height=4.5)
      DimPlot(epi.tumor, label = FALSE, group.by="pheno.sample", raster = TRUE)
      # + NoLegend()  
      dev.off()
    }
  ...
  ### barplot the percentage of each phenotype in cell types
  {
    ...
    table(epi.tumor@meta.data$pheno.sample, epi.tumor@meta.data$fig.patient)

    # get the percentage table  and colors for cell types
    state.sta=table(epi.tumor@meta.data$pheno.sample, epi.tumor@meta.data$celltype)
    state.sta=apply(state.sta, 1, function(x) 100*x/sum(x))
    state.sta=state.sta[names(color)[length(color):1], ]
    state.sta=state.sta[!rownames(state.sta)%in%c("Luminal","CellCycle"), ]
    temp.color=color[!names(color)%in%c("Luminal","CellCycle")]
    temp.color=temp.color[length(temp.color):1]
    ...
    require("scales")
    # color=hue_pal()(n_top)
    barplot( state.sta, xlab="", ylab="Percentage", col=temp.color, names.arg=colnames(state.sta), las=2, cex.names=0.5, beside=FALSE) # , xaxt="n"
    # las=2 : vertical x-names. las=1 : horizontal x-names 
    # facts=barN+1
    end_point = barN+8 #0.5 + n_top *facts-1
    # text(x=(c(1:barN)*1.2-1-0.35), y=0-0.35, cex=1, labels=colnames(state.sta), srt=45) # par("usr")[3] - 0, xpd=TRUE
    legend( end_point,100, rownames(state.sta), fill=temp.color, bty='n', cex = 0.8 );
    wilcox.test(state.sta["Clike", (colnames(state.sta)=="mHSPC")], state.sta["Clike", (colnames(state.sta)=="Primary PCa")])
    wilcox.test(state.sta["Clike", (colnames(state.sta)%in%c("CRPC", "NEPC"))], state.sta["Clike", (colnames(state.sta)=="Primary PCa")])
    # , bty='n': no-frame
    # text( x=seq(1,end_point,by=1)*1.2-0.2, y = state.sta[1,]/2, adj= c(1,1), xpd = TRUE, labels = paste(round(state.sta[1,],1),"%") , cex=0.7 )
    # text( x=seq(1,end_point,by=1)*1.2-0.2, y = 100-state.sta[2,]/2, adj= c(1,1), xpd = TRUE, labels = paste(round(state.sta[2,],1),"%") , cex=0.7 )
    dev.off() 
...
# check known epithelial cell types
check.genes=c("KLK3", "KRT8", "KRT13", "SCGB1A1", "KRT19", "KRT5", "CHGA", "SYP", "ENO2", "TP63", "AR", "nFeature_RNA", "nFeature_SCT", "KRT4", "TACSTD2", "PSCA", "monocle.state")
